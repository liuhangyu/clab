...
static inline block* new_block(block *meta)
{
  ...
  if (likely(success)) {
    ...
    int fd = open("/dev/zero", O_RDWR, 00777);
    b->buffer = mmap(0, block_size, PROT_READ | PROT_WRITE, MAP_PRIVATE , fd, 0);
...
static inline block* new_meta_block()
{
  ...
  //  fp = fopen("/dev/zero",  "w+" );
  int fd = open("/dev/zero", O_RDWR, 00777);
    char *buf = (char *)mmap(0, meta_block_size, PROT_READ | PROT_WRITE, MAP_PRIVATE , fd, 0);
